 ...
 *
 * Основная структура сопрограммы и её контекст.
 */
struct coro {
    /**
     * Coroutine state.
     ...
     *
     * Значение, возвращённое func.
     */
    void* ret;
    /**
     * Stack, used by the coroutine.
     *
     * Стек, используемый сопрограммой.
     */
    uint8_t* stack;
    /**
     ...
     *
     * Функция, вызываемая как сопрограмма.
     */
    coro_f func;
    /**
     * Last remembered coroutine context.
     *
     * Последний сохранённый контекст сопрограммы.
     */
    sigjmp_buf ctx;
    /**
     * Coroutine which is trying to join this one right now.
     *
     * Сопрограмма, которая сейчас пытается присоединиться к этой.
     */
    struct coro* joiner;
    /**
     * Links in a coroutine list, used by the scheduler.
     *
     * Связи в списке сопрограмм, используемом планировщиком.
     */
    struct rlist link;
};
...
struct coro_engine {
     ...
     *
     * Планировщик — основная сопрограмма: она представляет контекст,
     * в котором выполняется сам планировщик.
     */
    struct coro sched;
    /**
     * Which coroutine works at this moment.
     *
     * Какая сопрограмма выполняется сейчас.
     */
     ...
     *
     * Сопрограммы для выполнения на этой итерации цикла. Список
     * заполняется один раз в начале итерации.
     */
     ...
     * coros.
     *
     * Сопрограммы для выполнения на следующей итерации. Список
     * пополняется пробуждениями, yield и новыми сопрограммами.
     */
    struct rlist coros_running_next;
    /**
     * Joined coroutines to be reused.
     *
     * Присоединённые сопрограммы для повторного использования.
     */
    struct rlist coros_pool;
    /**
     * Total number of coroutines, including the pool.
     *
     * Общее число сопрограмм, включая пул.
     */
    size_t coro_count;
    /**
     ...
     *
     * Буфер, используемый конструктором сопрограммы, чтобы вернуться
     * из обработчика сигнала в конструктор и отменить sigaltstack и другое.
     */
    sigjmp_buf start_point;
};

static void coro_engine_create(struct coro_engine* engine) {
...
static void coro_engine_resume_next(struct coro_engine* engine) {
    ...
    if (sigsetjmp(from->ctx, 0) == 0) siglongjmp(to->ctx, 1);
    assert(rlist_empty(&from->link));
    assert(engine->this_coro == NULL);
    engine->this_coro = from;
}

static void coro_engine_suspend(struct coro_engine* engine) {
...
static void coro_engine_yield(struct coro_engine* engine) {
    struct coro* this_coro = engine->this_coro;
    assert(rlist_empty(&this_coro->link));
    assert(this_coro->state == CORO_STATE_RUNNING);
...
static void coro_engine_wakeup(struct coro_engine* engine, struct coro* coro) {
    if (coro->state == CORO_STATE_RUNNING) return;
    if (coro->state == CORO_STATE_FINISHED) return;
...
static void coro_engine_run(struct coro_engine* engine) {
    while (true) {
        assert(rlist_empty(&engine->coros_running_now));
        rlist_splice_tail(&engine->coros_running_now,
                          &engine->coros_running_next);
        if (rlist_empty(&engine->coros_running_now)) break;
         ...
         * loop.
         *
         * Добавить планировщик в хвост, чтобы управление вернулось
         * в конце этой итерации цикла.
         */
...
static void coro_engine_destroy(struct coro_engine* engine) {
    assert(engine->this_coro == NULL);
    assert(rlist_empty(&engine->coros_running_now));
 ...
 *
 * Основная часть создания сопрограммы: этот обработчик сигнала работает
 * на отдельном стеке через sigaltstack. При вызове он сохраняет текущий
 * контекст и возвращается в конструктор сопрограммы. Позже сопрограмма
 * продолжит выполнение отсюда.
 */
static void coro_body(int signum) {
     ...
     *
     * При вызове вернуться в конструктор сразу
     * после сохранения контекста.
     */
    if (sigsetjmp(c->ctx, 0) == 0) siglongjmp(my_engine->start_point, 1);
    /*
     * If the execution is here, then the coroutine should
     * finally start work.
     *
     * Если выполнение дошло сюда, сопрограмма должна
    ...
    while (true) {
        ...
        if (c->joiner != NULL) coro_engine_wakeup(my_engine, c->joiner);
        coro_engine_resume_next(my_engine);
        /*
         * Here it is restarted already, must have its
         * state restored.
         *
         * Здесь она уже перезапущена; её состояние
...
static struct coro* coro_engine_spawn_new(struct coro_engine* engine, coro_f func, void* func_arg) {
     ...
     *
     * Используется SIGUSR2. Сначала заблокируйте новые сигналы,
     * чтобы можно было установить новый обработчик.
     */
     ...
     *
     * Новый обработчик должен перейти на новый стек и сохранить эту позицию.
     * Затем стек отключается и становится выделенным этой сопрограмме.
     */
     ...
     *
     * Перейти на стек и сохранить его позицию.
     */
    ...
    if (sigsetjmp(engine->start_point, 1) == 0) {
        raise(SIGUSR2);
        while (engine->this_coro != NULL) sigsuspend(&suss);
    }
    assert(new_coro_engine == NULL);
     ...
     * used by it only.
     *
     * Вернуть старый стек и разблокировать SIGUSR2. Иными словами,
     * отменить все глобальные изменения. Новый стек теперь сохранён
     * только у новой сопрограммы и может использоваться только ею.
     */
     ...
     *
     * Теперь планировщик может работать с этой сопрограммой.
     */
    ++engine->coro_count;
...
static struct coro* coro_engine_spawn(struct coro_engine* engine, coro_f func, void* func_arg) {
    if (rlist_empty(&engine->coros_pool)) return coro_engine_spawn_new(engine, func, func_arg);
...
static void* coro_engine_join(struct coro_engine* engine, struct coro* coro) {
