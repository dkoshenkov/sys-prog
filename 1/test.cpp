...
static void send_start(struct ctx_send* ctx, struct coro_bus* bus, int channel,
                       unsigned data) {
    ctx->bus = bus;
    ctx->channel = channel;
    ctx->data = data;
...
static int send_join(struct ctx_send* ctx) {
...
static void recv_start(struct ctx_recv* ctx, struct coro_bus* bus, int channel,
                       unsigned* data) {
    ctx->bus = bus;
    ctx->channel = channel;
    ctx->data = data;
...
static int recv_join(struct ctx_recv* ctx) {
    unit_assert(coro_join(ctx->worker) == NULL);
    unit_assert(ctx->is_done);
    coro_bus_errno_set(ctx->err);
    return ctx->rc;
...
static void test_channel_reopen(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void test_send_blocking(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void test_send_blocking_recv_many(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    ...
    for (int i = 0; i < coro_count; ++i) send_start(&ctx[i], bus, c1, i + limit);
...
static void test_recv_blocking(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    int c1 = coro_bus_channel_open(bus, 3);
    unit_assert(c1 >= 0);
...
static void test_recv_blocking_send_many(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    const unsigned limit = 3;
    int c1 = coro_bus_channel_open(bus, limit);
    ...
    for (unsigned i = 0; i < coro_count; ++i) recv_start(&ctx[i], bus, c1, &datas[i]);
...
static void* stress_send_f(void* arg) {
...
static void* stress_recv_f(void* arg) {
...
static void* stress_send_recv_f(void* arg) {
...
static void test_stress_send_recv_concurrent(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void test_send_recv_very_many(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void test_wakeup_on_close(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void test_close_non_empty_bus(void) {
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void broadcast_start(struct ctx_broadcast* ctx, struct coro_bus* bus, unsigned data) {
    ctx->bus = bus;
    ctx->data = data;
    ctx->rc = -1;
    ctx->err = CORO_BUS_ERR_NONE;
...
static int broadcast_join(struct ctx_broadcast* ctx) {
    unit_assert(coro_join(ctx->worker) == NULL);
    coro_bus_errno_set(ctx->err);
    return ctx->rc;
}
...
static void test_broadcast_blocking_basic(void) {
#if NEED_BROADCAST
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void test_broadcast_blocking_drop_channel_during_wait(void) {
#if NEED_BROADCAST
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
static void test_send_vector_basic(void) {
#if NEED_BATCH
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    ...
    for (unsigned i = 1; i <= 5; ++i) unit_assert(coro_bus_recv(bus, c1, &data) == 0 && data == i);
    unit_assert(coro_bus_try_recv(bus, c1, &data) != 0);
    unit_assert(coro_bus_errno() == CORO_BUS_ERR_WOULD_BLOCK);
    coro_bus_channel_close(bus, c1);
    ...
    for (unsigned i = 1; i <= 7; ++i) unit_assert(coro_bus_recv(bus, c1, &data) == 0 && data == i);
    unit_assert(coro_bus_try_recv(bus, c1, &data) != 0);
    unit_assert(coro_bus_errno() == CORO_BUS_ERR_WOULD_BLOCK);
    coro_bus_channel_close(bus, c1);
...
static void send_v_start(struct ctx_send_v* ctx, struct coro_bus* bus, int channel,
...
static int send_v_join(struct ctx_send_v* ctx) {
    unit_assert(coro_join(ctx->worker) == NULL);
    unit_assert(ctx->is_done);
    coro_bus_errno_set(ctx->err);
    return ctx->rc;
...
static void test_send_vector_blocking(void) {
#if NEED_BATCH
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    int c1 = coro_bus_channel_open(bus, 3);
...
#if NEED_BATCH
static void* send_v_all_f(void* arg) {
    struct ctx_send_v* ctx = (decltype(ctx))arg;
    ctx->is_started = true;
    while (ctx->count > 0) {
...
static void send_v_all_start(struct ctx_send_v* ctx, struct coro_bus* bus, int channel,
                             const unsigned* data, unsigned count) {
    ctx->bus = bus;
    ctx->channel = channel;
    ctx->data = data;
...
static void test_send_vector_blocking_recv_many(void) {
#if NEED_BATCH
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    const unsigned limit = 3;
    ...
    for (unsigned i = 0; i < data_count; ++i) data[i] = i;
    unit_msg("start many coros");
    for (unsigned i = 0; i < coro_count; ++i) {
    ...
    for (unsigned i = 0; i < data_count; ++i) unit_assert(results[i]);
...
static void test_recv_vector_basic(void) {
#if NEED_BATCH
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    ...
    c1 = coro_bus_channel_open(bus, 10);
    unit_assert(c1 >= 0);
    for (unsigned i = 1; i <= 10; ++i) unit_assert(coro_bus_send(bus, c1, i) == 0);
    ...
    for (unsigned i = 7; i <= 10; ++i) unit_assert(coro_bus_recv(bus, c1, &data) == 0 && data == i);
    unit_assert(coro_bus_try_recv_v(bus, c1, data3, 3) < 0);
    unit_assert(coro_bus_errno() == CORO_BUS_ERR_WOULD_BLOCK);
    coro_bus_channel_close(bus, c1);
...
static void recv_v_start(struct ctx_recv_v* ctx, struct coro_bus* bus, int channel,
...
static int recv_v_join(struct ctx_recv_v* ctx) {
    unit_assert(coro_join(ctx->worker) == NULL);
    unit_assert(ctx->is_done);
    coro_bus_errno_set(ctx->err);
    return ctx->rc;
...
static void test_recv_vector_blocking(void) {
#if NEED_BATCH
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
...
#if NEED_BATCH
static void* recv_v_all_f(void* arg) {
    struct ctx_recv_v* ctx = (decltype(ctx))arg;
    ctx->is_started = true;
    while (ctx->count > 0) {
...
static void recv_v_all_start(struct ctx_recv_v* ctx, struct coro_bus* bus, int channel,
                             unsigned* data, unsigned count) {
    ctx->bus = bus;
    ctx->channel = channel;
    ctx->data = data;
...
static void test_recv_vector_blocking_recv_many(void) {
#if NEED_BATCH
    unit_test_start();
    struct coro_bus* bus = coro_bus_new();
    const unsigned limit = 3;
    ...
    unit_msg("send all the messages");
    for (unsigned i = 0; i < data_count; ++i) unit_assert(coro_bus_send(bus, c1, i) == 0);
