...
static uint32_t parse_token(const char* pos, const char* end, struct token* out) {
    ...
    while (pos < end) {
        ...
        case '"': if (quote == 0) {
            ...
            return pos + 1 - begin;
        case '\\': if (quote == '\'') goto append_and_next;
            if (quote == '"') {
                ++pos;
                if (pos == end) return 0;
                c = *pos;
                switch (c) {
                case '\\': goto append_and_next;
                case '\n': ++pos;
                    continue;
                case '"': goto append_and_next;
                default: break;
        ...
        case '&':
        case '|':
        case '>': if (quote != 0) goto append_and_next;
            if (!out->data.empty()) {
                out->type = TOKEN_TYPE_STR;
                return pos - begin;
            }
            ...
            if (*pos == c) {
                switch (c) {
                case '&': out->type = TOKEN_TYPE_AND;
                    break;
                case '|': out->type = TOKEN_TYPE_OR;
                    break;
                case '>': out->type = TOKEN_TYPE_OUT_APPEND;
                    break;
                default:
                    assert(false);
                    break;
                ...
                switch (c) {
                case '&': out->type = TOKEN_TYPE_BACKGROUND;
                    break;
                case '|': out->type = TOKEN_TYPE_PIPE;
                    break;
                case '>': out->type = TOKEN_TYPE_OUT_NEW;
                    break;
                default:
                    assert(false);
                    break;
        ...
        case ' ':
        case '\t':
        case '\r': if (quote != 0) goto append_and_next;
            assert(!out->data.empty());
            out->type = TOKEN_TYPE_STR;
            return pos + 1 - begin;
        case '\n': if (quote != 0) goto append_and_next;
            assert(!out->data.empty());
            out->type = TOKEN_TYPE_STR;
            return pos - begin;
        case '#': if (quote != 0) goto append_and_next;
...
enum parser_error parser_pop_next(struct parser* p, struct command_line** out) {
    ...
    while (pos < end) {
        ...
        if (used == 0) goto return_no_line;
        pos += used;
        expr e;
        switch (token.type) {
        case TOKEN_TYPE_STR: if (!line->exprs.empty() && line->exprs.back().type == EXPR_TYPE_COMMAND) {
        ...
        case TOKEN_TYPE_NEW_LINE:
            /* Skip new lines. */
            if (line->exprs.empty()) continue;
            goto close_and_return;
        case TOKEN_TYPE_PIPE: if (line->exprs.empty()) {
            ...
            line->exprs.emplace_back(std::move(e));
            continue;
        case TOKEN_TYPE_AND: if (line->exprs.empty()) {
            ...
            line->exprs.emplace_back(std::move(e));
            continue;
        case TOKEN_TYPE_OR: if (line->exprs.empty()) {
        ...
        case TOKEN_TYPE_BACKGROUND: goto close_and_return;
        default:
            assert(false);
        }
    }
...
close_and_return:
    if (token.type == TOKEN_TYPE_OUT_NEW || token.type == TOKEN_TYPE_OUT_APPEND) {
        if (token.type == TOKEN_TYPE_OUT_NEW) line->out_type = OUTPUT_TYPE_FILE_NEW;
        else line->out_type = OUTPUT_TYPE_FILE_APPEND;
        uint32_t used = parse_token(pos, end, &token);
        if (used == 0) goto return_no_line;
        pos += used;
...
return_error:
     ...
     *
     * Попытаться пропустить всю текущую строку. Её нельзя выполнить, но из-за
     * этого нельзя просто аварийно завершаться.
     */
    while (pos < end) {
        uint32_t used = parse_token(pos, end, &token);
        if (used == 0) break;
