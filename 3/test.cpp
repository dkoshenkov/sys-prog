...
static void test_io(void) {
    ...
    for (size_t i = 0; i < some_size; ++i) buffer[i] = 'a' + i % ('z' - 'a' + 1);
    fd1 = ufs_open("file", 0);
    unit_fail_if(fd1 == -1);
    size_t progress = 0;
    while (progress < some_size) {
        ssize_t to_write = progress % 123 + 1;
        if (to_write + progress > some_size) to_write = some_size - progress;
        ssize_t rc = ufs_write(fd1, buffer + progress, to_write);
        if (rc != to_write) break;
        progress += rc;
    }
    ...
    ufs_close(fd1);
    bool ok = true;
    for (size_t i = 0; i < some_size && ok; ++i) ok = ok && buffer[i] == (char)('a' + i % ('z' - 'a' + 1));
    ...
    assert(sizeof(buffer) > some_size);
    const int offset = 5;
    for (size_t i = 0; i < some_size; ++i) buffer[i] = 'a' + i % ('z' - 'a' + 1 + offset);
    fd1 = ufs_open("file", 0);
    unit_fail_if(fd1 == -1);
