...
static thread_task_f task_make_inc(int* arg) {
    return [arg]() { __atomic_add_fetch(arg, 1, __ATOMIC_RELAXED); };
}

static thread_task_f task_make_wait_for(int* arg) {
    return [arg]() {
        while (__atomic_load_n(arg, __ATOMIC_RELAXED) == 0) usleep(100);
    };
}
...
static void test_push(void) {
     ...
     */
    unit_check(thread_task_new(&t, task_make_inc(&arg)) == 0,
               "created new task");
    unit_check(thread_task_delete(t) == 0,
               "task can be deleted before push");
    unit_fail_if(thread_task_new(&t, task_make_inc(&arg)) != 0);
    /*
     * Bad join or delete.
     */
    unit_check(thread_task_join(t) == TPOOL_ERR_TASK_NOT_PUSHED,
    ...
    while (__atomic_load_n(&arg, __ATOMIC_RELAXED) != 1) usleep(100);
    unit_check(thread_task_join(t) == 0, "joined");
    unit_check(thread_task_delete(t) == 0, "deleted after join");
    /*
     * Re-push stress.
    ...
    for (int i = 0; i < count; ++i) {
        ...
        unit_fail_if(thread_task_new(tp, task_make_wait_for(&arg)) != 0);
        unit_fail_if(thread_pool_push_task(p, *tp) != 0);
    }
    __atomic_store_n(&arg, 1, __ATOMIC_RELAXED);
    for (int i = 0; i < count; ++i) {
        t = tasks[i];
        unit_fail_if(thread_task_join(t) != 0);
        if (i % 2 == 0)
...
static void test_thread_pool_delete(void) {
     ...
     */
    unit_fail_if(thread_pool_new(3, &p) != 0);
    unit_fail_if(thread_task_new(&t, [&m]() {
...
static void map_reduce_inc(struct thread_pool* p, struct thread_task** tasks, int count,
                           int* arg) {
    for (int i = 0; i < count; ++i) {
...
static void test_thread_pool_max_tasks(void) {
    ...
    for (int i = 0; i < TPOOL_MAX_TASKS; ++i) {
        struct thread_task** t = &tasks[i];
        unit_fail_if(thread_task_new(t, task_make_wait_for(&arg)) != 0);
        unit_fail_if(thread_pool_push_task(p, *t) != 0);
    }
    /*
...
static void test_timed_join(void) {
    ...
    int arg = 0;
    unit_fail_if(thread_pool_new(5, &p) != 0);
    unit_fail_if(thread_task_new(&task, task_make_wait_for(&arg)) != 0);
    unit_fail_if(thread_pool_push_task(p, task) != 0);
    unit_check(thread_task_timed_join(task, 0) ==
               TPOOL_ERR_TIMEOUT, "timed out on 0");

    struct timespec ts1, ts2;
    clock_gettime(CLOCK_MONOTONIC, &ts1);
    unit_check(thread_task_timed_join(task, 0.1) ==
    ...
    unit_check(thread_task_timed_join(task, -10000) ==
               TPOOL_ERR_TIMEOUT, "timeout out on negative timeout");

    clock_gettime(CLOCK_MONOTONIC, &ts1);
    __atomic_store_n(&arg, 1, __ATOMIC_RELAXED);
    unit_check(thread_task_timed_join(task, 1) == 0,
...
static void test_detach_stress(void) {
    ...
    for (int i = 0; i < 1000; ++i) {
        unit_fail_if(thread_task_new(&task, task_make_inc(&arg)) != 0);
        unit_fail_if(thread_pool_push_task(p, task) != 0);
        if (thread_task_detach(task) != 0) unit_check(false, "detach failed");
    }
    while (__atomic_load_n(&arg, __ATOMIC_RELAXED) != 1000) usleep(1000);
    /*
