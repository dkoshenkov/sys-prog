...
struct chat_client* chat_client_new(std::string_view name) {
     ...
     *
     * Игнорируйте параметр name, если не нужна поддержка за дополнительные 5 баллов.
     */
    (void)name;
...
int chat_client_connect(struct chat_client* client, std::string_view addr) {
     ...
     *
     * 1) Используйте getaddrinfo(), чтобы преобразовать addr в struct sockaddr_in.
     * 2) Создайте клиентский сокет функцией socket().
     * 3) Подключите его по найденному адресу функцией connect().
     */
    /* IMPLEMENT THIS FUNCTION */
    (void)client;
    (void)addr;
...
int chat_client_update(struct chat_client* client, double timeout) {
     ...
     *
     *
     * Проще всего ждать обновлений одного сокета с тайм-аутом через poll(). Epoll
     * удобен для множества сокетов, poll — для нескольких. Создайте и заполните
     * один struct pollfd, вызовите для него poll(), затем обработайте события
     * чтения и записи.
     */
    (void)client;
    (void)timeout;
