...
static size_t round_up(size_t original, std::align_val_t alignment) {
    size_t mult = static_cast<size_t>(alignment);
    assert(mult != 0);
    size_t rest = original % mult;
    if (rest == 0) return original;
...
static struct test_msg* test_msg_new(uint32_t len) {
    ...
    for (uint32_t i = TEST_MSG_ID_LEN; i < len; ++i) res->data[i] = 'a' + i % ('z' - 'a' + 1);
    res->data[len] = '\n';
    return res;
}
...
static void test_msg_check_data(const struct test_msg* msg, std::string_view data) {
...
static bool author_is_eq(const struct chat_message* msg, std::string_view name) {
#if NEED_AUTHOR
...
static void test_big_messages(void) {
    ...
    while (true) {
        ...
        if (rc1 == CHAT_ERR_TIMEOUT && rc2 == CHAT_ERR_TIMEOUT) break;
    }
...
static void test_multi_client(void) {
    ...
    while (true) {
        ...
        for (int i = 0; i < client_count; ++i) {
            if (chat_client_update(clis[i], 0) == 0) have_events = true;
        }
        if (chat_server_update(s, 0) == 0) have_events = true;
        if (!have_events) break;
    }
    ...
    for (int i = 0; i < client_count; ++i) chat_client_delete(clis[i]);
    delete[] clis;
    delete[] msg_counts;
    chat_server_delete(s);
    test_msg_delete(test_msg);
