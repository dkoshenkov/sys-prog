...
private:
    void priv_in_strand_on_resolve(
        const boost::system::error_code& err,
        chat_client_on_connect_f&& cb,
        boost::asio::ip::tcp::resolver::results_type results);

    void priv_in_strand_on_connect(
        const boost::system::error_code& err,
        chat_client_on_connect_f&& cb);
...
chat_client::chat_client(
    boost::asio::io_context& ioCtx,
    std::string_view name) : m_conn(std::make_shared<chat_client_peer>(ioCtx, name)) {
    // <YOUR CODE IF NEEDED>
}
...
void chat_client::connect_async(
    std::string_view endpoint,
    chat_client_on_connect_f&& cb) {
    m_conn->connect_async(endpoint, std::move(cb));
}

void chat_client::recv_async(
    chat_client_on_msg_f&& cb) {
    m_conn->recv_async(std::move(cb));
}

void chat_client::feed_async(
    std::string_view text) {
...
chat_client_peer::chat_client_peer(
    boost::asio::io_context& ioCtx,
    std::string_view name) : m_strand(ioCtx)
                             , m_sock(ioCtx)
                             , m_resolver(ioCtx)
                             , m_name(name) {
    // <YOUR CODE IF NEEDED>
...
chat_client_peer::~chat_client_peer() {
    for (std::unique_ptr<chat_client_request>& r : m_reqs) r->m_cb(CHAT_ERR_CANCELED, {});
    m_reqs.clear();

    // <YOUR CODE IF NEEDED>
}

void chat_client_peer::connect_async(
    std::string_view /* endpoint */,
    chat_client_on_connect_f&& /* cb */) {
    // <YOUR CODE IF NEEDED>
    abort();
...
void chat_client_peer::recv_async(
    chat_client_on_msg_f&& cb) {
    std::unique_ptr<chat_client_request> req =
...
void chat_client_peer::feed_async(
    std::string_view text) {
...
void chat_client_peer::priv_in_strand_on_resolve(
    const boost::system::error_code& /* err */,
    chat_client_on_connect_f&& /* cb */,
    boost::asio::ip::tcp::resolver::results_type /* results */) {
...
void chat_client_peer::priv_in_strand_on_connect(
    const boost::system::error_code& /* err */,
    chat_client_on_connect_f&& /* cb */) {
    assert(m_strand.running_in_this_thread());
...
void chat_client_peer::priv_in_strand_on_new_request(
    std::unique_ptr<chat_client_request> req) {
    assert(m_strand.running_in_this_thread());
    if (not m_reqs.empty()) {
...
void chat_client_peer::priv_in_strand_recv() {
    assert(m_strand.running_in_this_thread());
...
void chat_client_peer::priv_in_strand_on_recv(
    const boost::system::error_code& /* err */,
    std::size_t /* size */) {
    assert(m_strand.running_in_this_thread());
...
void chat_client_peer::priv_in_strand_on_new_feed(
    std::string_view /* text */) {
    assert(m_strand.running_in_this_thread());
...
void chat_client_peer::priv_in_strand_send() {
    assert(m_strand.running_in_this_thread());
...
void chat_client_peer::priv_in_strand_on_send(
    const boost::system::error_code& /* err */,
    std::size_t /* size */) {
    assert(m_strand.running_in_this_thread());
