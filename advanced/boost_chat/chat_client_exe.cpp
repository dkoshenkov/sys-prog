...
void chat_client_app::priv_read_next() {
    assert(m_strand.running_in_this_thread());
    boost::asio::async_read(m_input, boost::asio::buffer(m_in_buf, CHAT_RECV_BUF_SIZE),
                            boost::asio::bind_executor(m_strand,
                                                       std::bind(&chat_client_app::priv_on_input, this,
                                                                 std::placeholders::_1,
                                                                 std::placeholders::_2)));
}

void chat_client_app::priv_on_connect(
    chat_errcode err) {
    assert(m_strand.running_in_this_thread());
...
void chat_client_app::priv_on_recv(
    chat_errcode err,
    std::unique_ptr<chat_message> msg) {
    assert(m_strand.running_in_this_thread());
    if (err != CHAT_ERR_NONE) {
...
void chat_client_app::priv_on_input(
    const boost::system::error_code& err,
    size_t size) {
    assert(m_strand.running_in_this_thread());
    if (err) {
