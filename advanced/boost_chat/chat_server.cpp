...
private:
    void priv_in_strand_on_new_feed(
        std::string&& text);

    void priv_in_strand_recv();

    void priv_in_strand_on_recv(
        const boost::system::error_code& err,
        std::size_t size);

    void priv_in_strand_send();

    void priv_in_strand_on_send(
        const boost::system::error_code& err,
        std::size_t size);

    void priv_in_strand_stop();
...
private:
    void priv_in_strand_accept();

    void priv_in_strand_on_accept(
        const boost::system::error_code& err,
        boost::asio::ip::tcp::socket sock);

    void priv_in_strand_stop();

    void priv_in_strand_on_new_request(
        std::unique_ptr<chat_server_request> req);

    void priv_peer_on_recv(
        std::unique_ptr<chat_message> msg);

    void priv_in_strand_peer_on_recv(
        std::unique_ptr<chat_message> msg);

    void priv_peer_on_close(
        std::shared_ptr<chat_server_peer> peer);

    void priv_in_strand_peer_on_close(
        std::shared_ptr<chat_server_peer> peer);
...
chat_server::chat_server(
    boost::asio::io_context& ioCtx) : m_ctx(std::make_shared<chat_server_ctx>(ioCtx)) {
    // <YOUR CODE IF NEEDED>
}
...
chat_errcode chat_server::start(
    uint16_t port) {
    return m_ctx->start(port);
}

uint16_t chat_server::port() const {
    return m_ctx->port();
}

void chat_server::recv_async(
    chat_server_on_msg_f&& cb) {
    m_ctx->recv_async(std::move(cb));
}

void chat_server::feed_async(
    std::string_view text) {
...
chat_server_peer::chat_server_peer(
    boost::asio::ip::tcp::socket&& sock,
    std::shared_ptr<chat_server_ctx> server) : m_state(CHAT_SERVER_PEER_STATE_CONNECTED)
...
void chat_server_peer::priv_in_strand_on_new_feed(
    std::string&& /* text */) {
...
void chat_server_peer::priv_in_strand_recv() {
    assert(m_strand.running_in_this_thread());
    if (m_state == CHAT_SERVER_PEER_STATE_STOPPED) return;
...
void chat_server_peer::priv_in_strand_on_recv(
    const boost::system::error_code& /* err */,
    std::size_t /* size */) {
    assert(m_strand.running_in_this_thread());
...
void chat_server_peer::priv_in_strand_send() {
    assert(m_strand.running_in_this_thread());
    if (m_state == CHAT_SERVER_PEER_STATE_STOPPED) return;
...
void chat_server_peer::priv_in_strand_on_send(
    const boost::system::error_code& /* err */,
    std::size_t /* size */) {
    assert(m_strand.running_in_this_thread());
...
void chat_server_peer::priv_in_strand_stop() {
    assert(m_strand.running_in_this_thread());
    if (m_state != CHAT_SERVER_PEER_STATE_CONNECTED) return;
...
chat_server_ctx::chat_server_ctx(
    boost::asio::io_context& ioCtx) : m_state(CHAT_SERVER_STATE_NEW)
                                      , m_strand(ioCtx)
                                      , m_sock(ioCtx) {
    // <YOUR CODE IF NEEDED>
}
...
chat_errcode chat_server_ctx::start(
    uint16_t port) {
...
uint16_t chat_server_ctx::port() const {
    assert(m_state != CHAT_SERVER_STATE_NEW);
    return m_port;
}

void chat_server_ctx::stop() {
    boost::asio::post(m_strand, std::bind(
                          &chat_server_ctx::priv_in_strand_stop, shared_from_this()));
}

void chat_server_ctx::recv_async(
    chat_server_on_msg_f&& cb) {
    std::unique_ptr<chat_server_request> req =
...
void chat_server_ctx::priv_in_strand_on_accept(
    const boost::system::error_code& err,
    boost::asio::ip::tcp::socket sock) {
    assert(m_strand.running_in_this_thread());
    if (m_state == CHAT_SERVER_STATE_STOPPED) return;
    if (err) {
...
void chat_server_ctx::priv_in_strand_stop() {
    assert(m_strand.running_in_this_thread());
    if (m_state != CHAT_SERVER_STATE_LISTEN) return;
    m_state = CHAT_SERVER_STATE_STOPPED;
    m_sock.close();
    for (std::shared_ptr<chat_server_peer>& p : m_peers) p->stop();
    m_peers.clear();
}

void chat_server_ctx::priv_in_strand_on_new_request(
    std::unique_ptr<chat_server_request> req) {
    assert(m_strand.running_in_this_thread());
...
void chat_server_ctx::priv_peer_on_recv(
    std::unique_ptr<chat_message> msg) {
    boost::asio::post(m_strand, [ref = shared_from_this(), this,
...
void chat_server_ctx::priv_in_strand_peer_on_recv(
    std::unique_ptr<chat_message> msg) {
    assert(m_strand.running_in_this_thread());
...
void chat_server_ctx::priv_peer_on_close(
    std::shared_ptr<chat_server_peer> peer) {
    boost::asio::post(m_strand, [ref = shared_from_this(), this,
...
void chat_server_ctx::priv_in_strand_peer_on_close(
    std::shared_ptr<chat_server_peer> peer) {
    assert(m_strand.running_in_this_thread());
...
void chat_server_ctx::priv_in_strand_on_new_feed(
    std::string_view text) {
    assert(m_strand.running_in_this_thread());
    for (std::shared_ptr<chat_server_peer>& p : m_peers) p->feed_async(text);
}
