...
static int port_from_str(const char* str, uint16_t* port) {
    ...
    if (*end != 0) return -1;
    if (res > UINT16_MAX || res < 0) return -1;
    *port = (uint16_t)res;
    return 0;
}

chat_server_app::chat_server_app(
    uint16_t port) : m_strand(m_ioctx)
                     , m_server(m_ioctx)
...
void chat_server_app::priv_on_input(
    const boost::system::error_code& err,
    size_t size) {
    assert(m_strand.running_in_this_thread());
    if (err) {
