...
public:
    ...
    void stop() {
        m_backend.stop();
        for (std::unique_ptr<std::thread>& w : m_workers) w->join();
        m_workers.clear();
    }
...
struct test_msg final {
    void create(
        ...
        for (size_t i = TEST_MSG_ID_LEN; i < len; ++i) m_data[i] = 'a' + i % ('z' - 'a' + 1);
        m_data[len] = '\n';
    }
...
static void chat_message_extract_id(
    chat_message& msg,
    uint32_t* cli_id,
    uint32_t* msg_id) {
...
static inline std::string make_addr_str(
    uint16_t port) {
    return "localhost:" + std::to_string(port);
}

static std::unique_ptr<chat_message> server_recv_blocking(
    chat_server& server) {
    event ev;
    chat_errcode err;
...
static std::unique_ptr<chat_message> client_recv_blocking(
    chat_client& cli) {
    event ev;
    chat_errcode err;
    std::unique_ptr<chat_message> msg;
...
static chat_errcode client_connect_blocking(
    chat_client& cli,
...
static void test_big_messages() {
    ...
    chat_server_on_msg_f server_on_msg = [&](
        ...
        ++real_count;
        if (real_count == count) ev.send();
    };
    server.recv_async(chat_server_on_msg_f{server_on_msg});

    for (uint32_t i = 0; i < count; ++i) cli.feed_async(req.m_data);
...
static void test_stress_worker_f(
    io_core& core,
    test_stress_ctx& ctx) {
...
static void test_stress() {
    ...
    for (uint32_t i = 0; i < client_count; ++i) threads[i]->join();
}

static void test_big_author() {
    unit_test_start();
