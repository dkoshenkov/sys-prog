...
AsyncOperation::AsyncOperation(IOTask* sub) : myTask(sub) {
}

bool AsyncOperation::await_suspend(
...
AsyncRecv::AsyncRecv(
    IOTask* sub,
    void* data,
    size_t size) : AsyncOperation(sub)
                   , myData(data)
                   , mySize(size)
                   , myRes(-1) {
    execute();
...
void AsyncRecv::execute() {
    if ((myTask->myEventsReady & IO_EVENT_READ) == 0) return;
...
void AsyncSend::execute() {
    if ((myTask->myEventsReady & IO_EVENT_WRITE) == 0) return;
...
AsyncAccept::AsyncAccept(
    IOTask* sub,
    sockaddr* addr,
    socklen_t* size) : AsyncOperation(sub)
                       , myAddr(addr)
                       , mySize(size)
                       , myRes(-1) {
    execute();
...
IOTask::IOTask(
    IOCore& core,
    int fd) : myState(IO_TASK_STATE_NEW)
              , myFd(fd)
              , myIdx(-1)
              , myEventsReady(0)
              , myAsyncOp(nullptr)
...
IOCore::IOCore() : myFd(epoll_create1(0)) {
...
void IOCore::roll() {
    ...
    for (int i = 0; i < rc; ++i) {
        ...
        int mask = 0;
        if ((ev.events & EPOLLIN) != 0) mask |= IO_EVENT_READ;
        if ((ev.events & EPOLLOUT) != 0) mask |= IO_EVENT_WRITE;
        ...
        if (op != nullptr) {
            ...
            // spurious wakeup.
            if (!op->onIOEvent()) s->myAsyncOp = op;
        }
    }
}

void IOCore::processQueues() {
    if (mySize.load(std::memory_order_relaxed) == 0) return;
    std::unique_lock lock(myMutex);
