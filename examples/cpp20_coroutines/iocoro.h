...
struct IOCoroutinePromise {
    ...
    IOCoroutine get_return_object() { return {IOCoroutine::from_promise(*this)}; }

    std::suspend_never initial_suspend() noexcept { return {}; }

    IOCoroutineFinalizer final_suspend() noexcept { return {}; }
...
struct AsyncRecv final : public AsyncOperation {
    ...
    bool await_ready() const noexcept { return myRes >= 0; }

    // What co_await will return.
    ssize_t await_resume() { return myRes; }

private:
    void execute();

    bool onIOEvent() final;
...
struct AsyncConnect final : public AsyncOperation {
    ...
    bool await_ready() const noexcept { return myIsDone; }
...
public:
    ...
    IOCore& core() { return myCore; }

    //////////////////////////////////////////////
    // Those all are arguments for co_await.
    //
    AsyncRecv asyncRecv(void* data, size_t size) { return AsyncRecv(this, data, size); }

    AsyncSend asyncSend(const void* data, size_t size) { return AsyncSend(this, data, size); }

    AsyncAccept asyncAccept(sockaddr* addr, socklen_t* size) { return AsyncAccept(this, addr, size); }

    AsyncConnect asyncConnect(const sockaddr* addr, socklen_t size) { return AsyncConnect(this, addr, size); }
    //
    //////////////////////////////////////////////
...
struct IOCore {
    ...
    void stop() {
        myIsStopped.store(true, std::memory_order_relaxed);
        wakeup();
    }

    bool isStopped() const { return myIsStopped.load(std::memory_order_relaxed); }
