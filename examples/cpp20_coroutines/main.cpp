...
static void ioCoreRunF(
    IOCore& core);

static void makeFdNonblock(
    int fd);
...
static int run() {
    ...
    IOCore clientCore;
    for (int i = 0; i < theClientCount; ++i) (new Client(context))->connectAndRun(clientCore, port);
...
static void ioCoreRunF(IOCore& core) {
    while (!core.isStopped()) core.roll();
}
...
void Context::onClientFinish() {
    std::unique_lock lock(myMutex);
    ++myFinishClientCount;
    myCond.notify_one();
}

void Context::onServerFinish() {
    std::unique_lock lock(myMutex);
...
void Context::waitClientsFinish() {
    constexpr uint64_t target = theClientCount * 2;
    std::unique_lock lock(myMutex);
    while (myFinishClientCount < target) myCond.wait(lock);
    assert(myFinishClientCount == target);
}

void Context::waitServerFinish() {
    std::unique_lock lock(myMutex);
    while (!myIsServerFinished) myCond.wait(lock);
}

//////////////////////////////////////////////////////////////////////////////////////////

Client::Client(
    const std::shared_ptr<Context>& ctx) : myTask(nullptr)
                                           , myRecvCount(0)
                                           , mySendCount(0)
                                           , myContext(ctx) {
...
void Client::connectAndRun(
    ...
    [](Client* self, uint16_t port) -> IOCoroutine {
        ...
        int rc = co_await
        self->myTask->asyncConnect((sockaddr*)&addr, sizeof(addr));
        assert(rc == 0);
...
uint16_t Server::bindAndListenAndRun(
    IOCore& core) {
    sockaddr_in addr;
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
...
IOCoroutine Server::coroRun() {
    ...
    while (true) {
        ...
        socklen_t len = sizeof(addr);
        int sock = co_await
        task->asyncAccept((sockaddr*)&addr, &len);
        // Could be cancel.
        if (sock < 0) break;
