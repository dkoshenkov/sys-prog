...
static void coro_list_add(struct coro* c) {
    c->next = coro_list;
    c->prev = NULL;
    if (coro_list != NULL) coro_list->prev = c;
    coro_list = c;
}
...
void coro_yield(void) {
    ...
    if (to == NULL) coro_yield_to(&coro_sched);
    else coro_yield_to(to);
}
...
struct coro* coro_sched_wait(void) {
...
static void coro_body(int signum) {
     ...
     */
    if (sigsetjmp(c->ctx, 0) == 0) siglongjmp(start_point, 1);
    /*
...
struct coro* coro_new(coro_f func, void* func_arg) {
    ...
    if (sigsetjmp(start_point, 1) == 0) {
        raise(SIGUSR2);
        while (coro_this_ptr != NULL) sigsuspend(&suss);
    }
