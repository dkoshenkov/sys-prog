...
static void recursive(int deep, int id) {
    printf("%d: recursive step %d\n", id, deep);
    coro_yield();
    if (deep < 2) recursive(deep + 1, id);
    printf("%d: finish recursive step %d\n", id, deep);
}

static int coro_func(void* ptr) {
    int id = (int)ptr;
...
static int coro_tree_func(void* ptr) {
    int id = (int)ptr;
...
int main(void) {
    printf("Start main\n");
    coro_sched_init();
    for (int i = 0; i < coro_count; ++i) coros[i] = coro_new(coro_func, (void*)i);
    struct coro* c;
