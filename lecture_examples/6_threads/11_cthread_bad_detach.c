...
int cthread_join(volatile struct cthread* thread) {
    ...
    while (!thread->is_finished) sched_yield();
...
int cthread_runner(void* arg) {
    ...
    if (thread->is_detached) cthread_destroy(thread);
    thread->is_finished = true;
    return 0;
}

void cthread_detach(struct cthread* thread) {
    if (thread->is_finished) cthread_destroy(thread);
    sleep(1);
...
void cthread_exit(struct cthread* thread, int retcode) {
