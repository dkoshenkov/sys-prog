...
static void futex_wake(int* val) {
    syscall(SYS_futex, val, FUTEX_WAKE, 1, NULL, NULL, 0);
}

static void futex_wait(int* val, int old) {
...
static void condvar_signal(struct condvar* cond) {
...
static void condvar_wait(struct condvar* cond, pthread_mutex_t* mutex) {
...
static void condvar_create(struct condvar* cond) {
    cond->sequence = 0;
}
...
static void* thread_send_f(void* arg) {
    ...
    while (state->counter_send < state->target) {
        while (state->counter_send > state->counter_recv) condvar_wait(&state->send_cond, &state->mutex);
        ++state->counter_send;
...
static void* thread_recv_f(void* arg) {
    struct state* state = arg;
    ...
    while (state->counter_recv < state->target) {
        while (state->counter_send == state->counter_recv) condvar_wait(&state->recv_cond, &state->mutex);
...
int main() {
    ...
    for (int i = 0; i < thread_count; ++i) pthread_create(&tid_send[i], NULL, thread_send_f, &state);

    pthread_t tid_recv[thread_count];
    for (int i = 0; i < thread_count; ++i) pthread_create(&tid_recv[i], NULL, thread_recv_f, &state);
