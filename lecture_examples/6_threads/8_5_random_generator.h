...
static unsigned int mersenne_twister_generate(struct mersenne_twister* twister) {
    int i = twister->index;
    int i2 = twister->index + 1;
    int j = twister->index + MT_IA;
    if (i2 >= MT_LEN) i2 = 0;
    if (j >= MT_LEN) j -= MT_LEN;
...
static void mersenne_twister_create(struct mersenne_twister* twister, unsigned int seed) {
    for (int i = 0; i < MT_LEN; i++) twister->buffer[i] = seed;
    twister->index = 0;
    for (int i = 0; i < MT_LEN * 100; i++) mersenne_twister_generate(twister);
}
