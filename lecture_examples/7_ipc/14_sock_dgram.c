...
void initiator_f(int sock, struct sockaddr_un* next_addr) {
    int number;
...
void worker_f(int sock, struct sockaddr_un* next_addr) {
    while (1) {
        int buffer = 0;
        ssize_t size = recv(sock, &buffer, sizeof(buffer), 0);
        if (size == -1) {
...
int main(int argc, const char** argv) {
    ...
    ssize_t size;
    int buffer;
    if (myid == 0) initiator_f(sock, &next_addr);
    else worker_f(sock, &next_addr);
    return 0;
}
