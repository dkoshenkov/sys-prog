...
void write_to_shared_mem(char* mem, const char* src, int size) {
    ...
    char* saved_mem = mem;
    while (1) {
        while (!__atomic_load_n(is_writable, __ATOMIC_ACQUIRE)) sched_yield();
...
void read_from_shared_mem(char* mem, char* dst, int size) {
    volatile char* is_readable = &mem[IS_READABLE];
    volatile char* is_writable = &mem[IS_WRITABLE];
    mem += MEM_META_SIZE;
    int mem_size = MEM_SIZE - MEM_META_SIZE;
    ...
    char* saved_mem = mem;
    while (1) {
        while (!__atomic_load_n(is_readable, __ATOMIC_ACQUIRE)) sched_yield();
        int to_copy = mem_size > size ? size : mem_size;
