...
int write_symbols(int fd, char* cmd) {
    int n;
    char c;
    sscanf(cmd, "%d %c", &n, &c);
    for (int i = 0; i < n; ++i) {
        if (write(fd, &c, sizeof(c)) == -1) return -1;
    }
    return 0;
}

int char_to_whence(char whence) {
    if (whence == 's') return SEEK_SET;
    if (whence == 'e') return SEEK_END;
    return SEEK_CUR;
}

int char_to_type(char type) {
    if (type == 'r') return F_RDLCK;
    if (type == 'w') return F_WRLCK;
    return F_UNLCK;
}
...
int get_lock(int fd, char* cmd) {
    ...
    if (fl.l_type == F_UNLCK) printf("no lock on this region\n");
    else printf("process %d holds the lock\n", (int)fl.l_pid);
    return 0;
}
